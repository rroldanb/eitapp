...
if proyecto:
    PAGES.append(('GET', f'/red-vial/proyecto/{proyecto.id}/analisis-flujos/', {}, 'analisis_flujos'))
...
for method, path, data, label in PAGES:
    try:
        ...
        results.append({
            'label': label,
            'status': f'ERR: {e}',
            'inline_scripts': 0, 'inline_bytes': 0,
            'external_count': 0, 'external_bytes': 0,
            'total_js_bytes': 0, 'total_scripts': 0,
        })
...
for r in results:
    ...
    print(f'{r["label"]:<28} {str(r["status"]):<8} {inline_kb:<8.1f} {r["external_count"]:<10} {ext_kb:<10.1f} {total_kb:<10.1f}')
...
print(f'{"TOTAL / AVG":<28} {"":<8} {total_inline_kb:<8.1f} {avg_refs:<10.1f} {total_ext_kb:<10.1f} {total_js_kb:<10.1f}')
print(f'')
