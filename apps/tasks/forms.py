    ...
    def __init__(self, *args, **kwargs):
        ...
        if user:
            ...
            if role == Role.ENCUESTADOR:
                ...
                self.fields["assignee"].queryset = user.__class__.objects.filter(
                    profile__role=Role.ENCUESTADOR
                )
...
class BulkTaskForm(forms.Form):
    titles = forms.CharField(
        ...
        widget=forms.Textarea(
            attrs={"rows": 5, "class": "form-control", "placeholder": "Una tarea por línea"}
        ),
    )
