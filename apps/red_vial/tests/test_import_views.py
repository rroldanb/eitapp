...
class ImportViewsAuthTest(TestCase):
    ...
    def test_import_execute_success(self):
        validation = {
            'results': {
                'Mandante': {'valid': [{'name': 'M1', 'location': 'L1', 'details': ''}], 'duplicates': [], 'errors': [], 'sheet': 'Mandante', 'total': 1},
            },
    ...
    def test_import_execute_with_duplicate_update(self):
        ...
        validation = {
            'results': {
                'Mandante': {'valid': [], 'duplicates': [{'name': 'ExistingM', 'location': 'NewLoc', 'details': ''}], 'errors': [], 'sheet': 'Mandante', 'total': 1},
            },
            'total_valid': 0, 'total_errors': 0, 'total_duplicates': 1,
        }
        session = self.client.session
    ...
    def test_import_execute_exception(self):
        ...
        session['import_validation'] = validation
        session.save()
        with patch('apps.red_vial.views.import_views.execute_import', side_effect=ValueError('Boom!')):
            response = self.client.post(self._url('execute'))
            self.assertEqual(response.status_code, 200)
