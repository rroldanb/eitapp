...
from apps.red_vial.services.import_service import (
    ...
    _resolve_arco,
    _resolve_fk,
    _resolve_puntocontrol_por_nombre,
    _restore_fk_values,
    _sanitize_for_session,
    _trim_row,
    _VirtualObj,
    execute_import,
    parse_excel,
...
class ValidateSelectionTest(TestCase):
    ...
    def test_empty_selected_sheets(self):
        result = validate_selection(
            {"Mandante": [{"name": "M1", "location": "L1"}]}, [], self.proyecto
        )
...
class ExecuteImportTest(TestCase):
    ...
    def test_insert_mandante(self):
        validation = self._make_validation_result(
            {
                "Mandante": {
                    "valid": [{"name": "NewMandante", "location": "Loc", "details": ""}],
                    "duplicates": [],
                    "errors": [],
                    "sheet": "Mandante",
                },
    ...
    def test_update_existing_mandante(self):
        ...
        report = execute_import(
            validation, self.proyecto, self.user, update_duplicates={"Mandante": True}
        )
    ...
    def test_skip_duplicates(self):
        ...
        report = execute_import(
            validation, self.proyecto, self.user, update_duplicates={"Mandante": False}
        )
        mandante = Mandante.objects.get(name="ExistingM")
    ...
    def test_insert_calle(self):
        validation = self._make_validation_result(
            {
                "Calle": {
                    "valid": [{"nombre": "NuevaCalle", "numero": 99, "proyecto": self.proyecto}],
                    "duplicates": [],
                    "errors": [],
                    "sheet": "Calle",
                },
    ...
    def test_insert_nodo(self):
        validation = self._make_validation_result(
            {
                "Nodo": {
                    "valid": [{"numero": 10, "interseccion": "Test", "proyecto": self.proyecto}],
                    "duplicates": [],
                    "errors": [],
                    "sheet": "Nodo",
                },
    ...
    def test_insert_arco(self):
        validation = self._make_validation_result(
            {
                "Arco": {
                    "valid": [
                        {
                            "nodo_origen": self.nodo1,
                            "nodo_destino": self.nodo2,
                            "longitud": 100.0,
        ...
        report = execute_import(validation, self.proyecto, self.user)
        self.assertTrue(
            Arco.objects.filter(nodo_origen=self.nodo1, nodo_destino=self.nodo2).exists()
        )
    ...
    def test_insert_regulacion(self):
        validation = self._make_validation_result(
            {
                "Regulacion": {
                    "valid": [{"codigo": "PARE", "descripcion": "Pare"}],
                    "duplicates": [],
                    "errors": [],
                    "sheet": "Regulacion",
                },
    ...
    def test_insert_punto_control(self):
        arco = Arco.objects.create(
            nodo_origen=self.nodo1, nodo_destino=self.nodo2, longitud=50.0, proyecto=self.proyecto
        )
        validation = self._make_validation_result(
            {
                "PuntoControl": {
    ...
    def test_re_resolve_fk_string_values(self):
        ...
        Mandante.objects.create(name="ReResolveM", location="Loc")
        validation = self._make_validation_result(
            {
                "Contacto": {
                    "valid": [{"name": "Contact1", "email": "c@c.com", "mandante": "ReResolveM"}],
                    "duplicates": [],
                    "errors": [],
                    "sheet": "Contacto",
                },
        ...
        validation = self._make_validation_result(
            {
                "Mandante": {
                    "valid": [],
                    "duplicates": [],
                    "errors": [{"row": 1, "data": {}, "errors": ["'name' es requerido"]}],
                    "sheet": "Mandante",
                },
            }
        )
    ...
    def test_multiple_sheets_processed(self):
        ...
        validation = self._make_validation_result(
            {
                "Mandante": {
                    "valid": [{"name": "M1", "location": "Loc", "details": ""}],
                    "duplicates": [],
                    "errors": [],
                    "sheet": "Mandante",
                    "total": 1,
...
class ResolveArcoTest(TestCase):
    def setUp(self):
        ...
        self.nodo1 = Nodo.objects.create(numero=1, proyecto=self.proyecto)
        self.nodo2 = Nodo.objects.create(numero=2, proyecto=self.proyecto)
        self.arco = Arco.objects.create(
            nodo_origen=self.nodo1, nodo_destino=self.nodo2, longitud=50, proyecto=self.proyecto
        )
...
class ResolveFkTest(TestCase):
    ...
    def test_resolve_cached(self):
        ctx = {
            "proyecto": self.proyecto,
            "resolved": {"Mandante:TestMandante": self.mandante},
            "shared_cache": {},
        }
        result = _resolve_fk("Contacto", "mandante", "TestMandante", ctx)
        self.assertEqual(result, self.mandante)
    ...
    def test_resolve_via_shared_cache(self):
        ...
        ctx = {
            "proyecto": self.proyecto,
            "resolved": {},
            "shared_cache": {("Nodo", "numero", "999", str(self.proyecto.id)): True},
        }
...
class ParseRowTest(TestCase):
    ...
    def test_parse_mandante(self):
        result = _parse_row(
            "Mandante", {"name": "M1", "location": "L1", "details": "D1"}, 1, self.ctx, {}
        )
    ...
    def test_parse_contacto_valid(self):
        ...
        result = _parse_row(
            "Contacto", {"name": "C1", "email": "c@c.com", "mandante": "TM"}, 1, self.ctx, {}
        )
    ...
    def test_parse_contacto_missing_mandante(self):
        result = _parse_row(
            "Contacto",
            {"name": "C1", "email": "c@c.com", "mandante": "NonExistent"},
            1,
            self.ctx,
            {},
        )
    ...
    def test_parse_proyecto_valid(self):
        ...
        result = _parse_row(
            "Proyecto",
            {"title": "P1", "date_started": "15/01/2024", "mandante": "TM"},
            1,
            self.ctx,
            {},
        )
    ...
    def test_parse_calle_valid(self):
        self.ctx["resolved"]["Proyecto:TP"] = self.proyecto
        result = _parse_row(
            "Calle", {"nombre": "Calle1", "numero": "10", "proyecto": "TP"}, 1, self.ctx, {}
        )
    ...
    def test_parse_nodo_valid(self):
        self.ctx["resolved"]["Proyecto:TP"] = self.proyecto
        result = _parse_row(
            "Nodo", {"numero": "10", "interseccion": "Test", "proyecto": "TP"}, 1, self.ctx, {}
        )
        self.assertEqual(result["data"]["numero"], 10)
        self.assertEqual(result["data"]["proyecto"], self.proyecto)
    ...
    def test_parse_arco_valid(self):
        ...
        result = _parse_row(
            "Arco",
            {"nodo_origen": "1", "nodo_destino": "2", "longitud": "100.5", "proyecto": "TP"},
            1,
            self.ctx,
            {},
        )
    ...
    def test_parse_regulacion(self):
        result = _parse_row(
            "Regulacion", {"codigo": "SEM", "descripcion": "Semáforo"}, 1, self.ctx, {}
        )
    ...
    def test_parse_fase_semaforica(self):
        ...
        pc = PuntoControl.objects.create(
            movimiento="12",
            nodo=nodo_pc,
            arco_entrada=self.arco,
            arco_salida=self.arco,
...
class ExecuteSheetTest(TestCase):
    ...
    def test_mandante_get_or_create_new(self):
        ...
        _execute_sheet(
            "Mandante",
            [{"name": "NewM", "location": "Loc", "details": ""}],
            self.proyecto,
            self.user,
            report,
        )
    ...
    def test_mandante_get_or_create_existing(self):
        ...
        _execute_sheet(
            "Mandante",
            [{"name": "ExistingM", "location": "NewLoc", "details": "Updated"}],
            self.proyecto,
            self.user,
            report,
        )
...
class FullPipelineIntegrationTest(TestCase):
    ...
    def _make_sheets_data(self, mandante_name, proyecto_title):
        sheets = {
            ...
            "Contacto": [
                {
                    ...
                    "position": "",
                    "details": "",
                    "mandante": mandante_name,
                },
            ],
            "Proyecto": [
                {
                    "title": proyecto_title,
                    "description": "",
                    "date_started": "01/01/2025",
                    "mandante": mandante_name,
                },
            ],
            "Calle": [
            ...
            "Nodo": [
                {
                    "numero": 1,
                    "interseccion": "Calle A con Calle B",
                    "calle_1": "Calle A",
                    "calle_2": "Calle B",
                    "numero_pc": 1,
                    "plano": "",
                    "imagen": "",
                    "proyecto": proyecto_title,
                },
                {
                    "numero": 2,
                    "interseccion": "Calle B con Calle A",
                    "calle_1": "Calle B",
                    "calle_2": "Calle A",
                    "numero_pc": 2,
                    "plano": "",
    ...
    def test_full_pipeline_import_in_existing_project(self):
        ...
        if validation["total_errors"] > 0:
            ...
            self.fail(
                f"Validation errors: {validation['total_errors']} — {'; '.join(error_details)}"
            )
        ...
        self.assertTrue(
            CoeficienteCruce.objects.filter(nomenclatura="VL", proyecto__isnull=True).exists()
        )
        self.assertTrue(
            CoeficienteCruce.objects.filter(nomenclatura="BUS", proyecto=self.proyecto).exists()
        )
        self.assertTrue(Periodo.objects.filter(codigo="PM-L", proyecto=self.proyecto).exists())
    ...
    def test_template_generar_plantilla_is_parseable(self):
        ...
        self.assertEqual(
            parsed_sheets, expected_sheets, f"Missing sheets: {expected_sheets - parsed_sheets}"
        )
