...
FK_FIELDS = {
    ...
    "Nodo": {
        "calle_1": ("Calle", "nombre"),
        "calle_2": ("Calle", "nombre"),
        "proyecto": ("Proyecto", "title"),
    },
    "Arco": {
        "nodo_origen": ("Nodo", "numero", "proyecto_id"),
        "nodo_destino": ("Nodo", "numero", "proyecto_id"),
        "proyecto": ("Proyecto", "title"),
    },
    "CoeficienteCruce": {"proyecto": ("Proyecto", "title")},
    "Periodo": {"proyecto": ("Proyecto", "title")},
    "PuntoControl": {
        "nodo": ("Nodo", "numero", "proyecto_id"),
        "regulacion": ("Regulacion", "codigo"),
        "proyecto": ("Proyecto", "title"),
    },
    "Periodizacion": {
        "pc": ("PuntoControl", "nombre", "proyecto_id"),
        "periodo": ("Periodo", "codigo", "proyecto_id"),
        "proyecto": ("Proyecto", "title"),
    },
    "ParametroArco": {
        "punto_control": ("PuntoControl", "nombre", "proyecto_id"),
        "proyecto": ("Proyecto", "title"),
    },
    "FaseSemaforica": {
        "punto_control": ("PuntoControl", "nombre", "proyecto_id"),
...
def validate_selection(
    ...
    for sheet_name in sheets_in_order:
        ...
        for _other_sheet, fk_dict in FK_FIELDS.items():
            for _field_name, fk_info in fk_dict.items():
        ...
        if sheet_name == "Arco":
            for row_data in res["valid"]:
                ...
                if origen is not None and destino is not None:
                    ...
                    sc_key = (
                        ("Arco", "_key", arco_key, str(proyecto.id))
                        if proyecto
                        else ("Arco", "_key", arco_key)
                    )
...
def execute_import(validation_result, proyecto, user, update_duplicates=None):
    ...
    for sheet_name in SHEET_ORDER:
        ...
        if sheet_name == "PuntoControl":
            for row_data in res["valid"] + res.get("duplicates", []):
                row_proyecto = row_data.get("proyecto")
                arco_ctx = row_proyecto if isinstance(row_proyecto, Proyecto) else proyecto
                for field in ("arco_entrada", "arco_salida"):
                    val = row_data.get(field)
                    if isinstance(val, _VirtualObj):
                        val = val._value
    ...
    for data in rows:
        try:
            with transaction.atomic():
                if sheet_name == "Mandante":
                    obj, created = Mandante.objects.get_or_create(
                        name=data["name"],
                        defaults={"location": data["location"], "details": data["details"]},
                    )
                    if not created:
                    ...
                    try:
                        ...
                        Calle.objects.create(
                            nombre=data["nombre"], numero=data["numero"], proyecto=data["proyecto"]
                        )
                        sheet_report["inserted"] += 1
                    ...
                    try:
                        obj = Arco.objects.get(
                            nodo_origen=data["nodo_origen"],
                            nodo_destino=data["nodo_destino"],
                    ...
                    obj, created = Regulacion.objects.get_or_create(
                        codigo=data["codigo"], defaults={"descripcion": data["descripcion"]}
                    )
                    if not created:
                    ...
                    if isinstance(pc, PuntoControl) and pc.arco_salida_id and pc.arco_entrada_id:
                        ...
                        sheet_report["rejected"].append(
                            {
                                "row": data.get("_original_row", "?"),
                                "reason": "PuntoControl sin arcos de entrada/salida",
                            }
                        )
                        continue
                    ...
                    try:
                        obj = FaseSemaforica.objects.get(
                            punto_control=data["punto_control"], fase_numero=data["fase_numero"]
                        )
                    ...
                    obj, created = ConfiguracionTransyt.objects.get_or_create(
                        ...
                        defaults={
                            ...
                            "W": data["W"],
                            "K": data["K"],
                            "perdida_inicial": data["perdida_inicial"],
                            "ganancia_final": data["ganancia_final"],
                        },
                    )
                    if not created:
            ...
            sheet_report["rejected"].append(
                {"row": data.get("_original_row", "?"), "reason": str(e)}
            )
...
def filter_by_dataset(parsed_data, dataset_title):
    ...
    for sheet_name, rows in parsed_data.items():
        ...
        if sheet_name == "Proyecto":
            filtered[sheet_name] = [
                r for r in rows if (r.get("title") or "").strip() == dataset_title
            ]
        elif proyecto_key:
            filtered[sheet_name] = [
                r for r in rows if (r.get(proyecto_key) or "").strip() == dataset_title
            ]
        else:
