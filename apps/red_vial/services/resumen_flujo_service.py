...
def recalcular_resumenes(proyecto_id):
    ...
    for agg in aggs:
        ...
        obj, created = ResumenFlujo.objects.update_or_create(
            pc_id=agg['pc'],
...
def get_analisis_flujos(proyecto_id, pc_ids=None, periodo_ids=None, fecha=None):
    ...
    if fecha:
        qs = Periodizacion.objects.filter(
            pc__proyecto_id=proyecto_id, fecha=fecha
        )
        if pc_ids:
        ...
        rows = qs.values(
            'pc', 'periodo', 'flujo_total', 'promedio', 'num_registros'
        )
    ...
    pcs = {str(p.id): p for p in PuntoControl.objects.filter(id__in=pc_ids_set).select_related('nodo')}
...
def get_comparison(analisis_data, periodos_ordered):
    ...
    result = []
    for pc_id, data in comparison_map.items():
        row = {
