...
class ConfiguracionTransyt(BaseModel):
    ...
    ciclo = models.PositiveIntegerField(default=60, help_text="Tiempo de ciclo en segundos")
    W = models.FloatField(default=10.0, help_text="Costo por hora de demora ($/hr-veh) — Tarjeta 1")
    K = models.FloatField(default=0.5, help_text="Costo por detención ($/det) — Tarjeta 1")
...
class ParametroArco(BaseModel):
    proyecto = models.ForeignKey(Proyecto, on_delete=models.CASCADE, related_name="parametros_arco")
...
class FaseSemaforica(BaseModel):
    ...
    fase_numero = models.PositiveSmallIntegerField(help_text="Número de fase semafórica")
