...
class ResumenFlujo(BaseModel):
    ...
    promedio = models.FloatField(
        blank=True, null=True, help_text="Promedio de ftot por registro de 15 min"
    )
    num_registros = models.IntegerField(
        blank=True, null=True, help_text="Número de registros de Periodización agregados"
    )
    ...
    def calcular_flujo(self) -> int:
        total = (
            Periodizacion.objects.filter(
                pc=self.pc,
                periodo=self.periodo,
            ).aggregate(suma=Sum("ftot"))["suma"]
            or 0
        )
