...
class Nodo(BaseModel):
    ...
    calle_1 = models.ForeignKey(
        Calle, on_delete=models.SET_NULL, null=True, blank=True, related_name="nodos_calle_1"
    )
    calle_2 = models.ForeignKey(
        Calle, on_delete=models.SET_NULL, null=True, blank=True, related_name="nodos_calle_2"
    )
    proyecto = models.ForeignKey(Proyecto, on_delete=models.CASCADE, related_name="nodos")
    # is_pc = models.BooleanField(default=False)  # Indica si es un Punto de control (PC)
    numero_pc = models.PositiveSmallIntegerField(
        blank=True,
        null=True,
        unique=False,
