...
from apps.imagenes.services.storage_service import delete_project_image, upload_project_image
...
from apps.red_vial.services.generador_dat import (
    DatGenerator,
    generar_fases_semaforicas,
    generar_parametros_arco,
)
...
def proyecto_delete_view(request, proyecto_id):
    try:
        ...
        return render(
            request,
            "proyectos.html",
            {"error": str(e), "proyectos": get_all_proyectos(), "list_title": "Proyectos"},
        )
...
def proyecto_generar_dat_view(request, proyecto_id):
    ...
    periodo = (
        Periodo.objects.filter(id=periodo_id, proyecto=proyecto).first() if periodo_id else None
    )
